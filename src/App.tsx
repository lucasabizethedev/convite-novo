...
export default function App() {
    ...
    const coresPresentes = ["Preto", "Branco", "Amadeirado", "Inox"];
    ...
    useEffect(() => {
        ...
        const atualizarContador = () => {
            // AJUSTADO DE VOLTA PARA 13:00:00
    ...
    const confirmar = async () => {
        if (!nome.trim() || !presente || !corSelecionada) {
            alert("Por favor, preencha o seu nome e selecione o presente e a COR! 🎁");
            return;
        }
        ...
        try {
            const res = await fetch("https://sheetdb.io/api/v1/6eww570s2ed6h", {
                ...
                body: JSON.stringify({
                    data: [{
                        ...
                        presente: `${presente} (Cor: ${corSelecionada})`,
                        confirmado: 'SIM',
            ...
            if (res.ok) {
                alert("Presença confirmada! Obrigado pelo carinho. ❤️");
    ...
    return (
        <div className="container">
            ...
            <a href="https://www.google.com/maps/search/?api=1&query=R.+Oldemar+de+Souza,+23+-+Jóquei+Clube" target="_blank" rel="noreferrer" className="btn-mapa">
                📍 Ver Localização
            </a>
            ...
            <div className="card-main">
                <p style={{ fontSize: '0.65rem', color: '#a67c52', fontWeight: 600, letterSpacing: '2px' }}>CHÁ DE PANELA & NOIVADO</p>
                <h1>Camilly & Lucas</h1>
                ...
                <div className="form-group">
                    ...
                    <select value={acompanhantes} onChange={e => setAcompanhantes(e.target.value)}>
                        ...
                        {[1,2,3,4,5,6,7].map(n => <option key={n} value={n}>+ {n} {n === 1 ? 'pessoa' : 'pessoas'}</option>)}
                    </select>
                    ...
                    <button className="btn-enviar" onClick={confirmar} disabled={carregando}>
                        {carregando ? "A confirmar..." : "Confirmar Presença"}
                    </button>
                </div>
            </div>
